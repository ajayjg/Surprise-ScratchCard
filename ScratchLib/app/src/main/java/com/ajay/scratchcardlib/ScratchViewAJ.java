package com.ajay.scratchcardlib;
...
/**
 * This view start with customized color bitmap and onTouch to make it
 * transparent
 * 
 * @author Ajay
 */
public class ScratchViewAJ extends SurfaceView implements IScratchViewAj, SurfaceHolder.Callback {
	private static final String TAG = "AJScratchView";
	...
	public ScratchViewAJ(Context ctx, AttributeSet attrs) {
		super(ctx, attrs);
		init(ctx, attrs);
	}

	public ScratchViewAJ(Context context) {
		super(context);
		init(context, null);
	}
	...
	private void init(Context context, AttributeSet attrs) {
		...
		TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.IScratchViewAj, 0, 0);

		final int indexCount = ta.getIndexCount();
		for (int i = 0; i < indexCount; i++) {
			int attr = ta.getIndex(i);

			if (attr == R.styleable.IScratchViewAj_overlayColor) {
				mOverlayColor = ta.getColor(attr, DEFAULT_COLOR);
			} else if (attr == R.styleable.IScratchViewAj_revealSize) {
				mRevealSize = ta.getDimensionPixelSize(attr, DEFAULT_REVEAL_SIZE);
			} else if (attr == R.styleable.IScratchViewAj_antiAlias) {
				mIsAntiAlias = ta.getBoolean(attr, false);
			} else if (attr == R.styleable.IScratchViewAj_scratchable) {
				mIsScratchable = ta.getBoolean(attr, true);
			} else if (attr == R.styleable.IScratchViewAj_scratchDrawable) {
				mScratchDrawable = ta.getDrawable(R.styleable.IScratchViewAj_scratchDrawable);
			}
	...
	class WScratchViewThread extends Thread {
		...
		private ScratchViewAJ mView;
		private boolean mRun = false;

		public WScratchViewThread(SurfaceHolder surfaceHolder, ScratchViewAJ view) {
    ...
    //Set the AJScartchView clickable
    @Override
